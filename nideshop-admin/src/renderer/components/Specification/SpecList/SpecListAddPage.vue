      ...
      <div class="form-table-box">
        ...
        >
          <el-form-item label="规格所属分类">
            <p>
              所属分类暂时只支持一种,未选取前可以任意选择，选中一个后以这个为准，重置请删除当前分类所有规格重新添加
            </p>
            <p />
            <el-select
              :options="options"
              placeholder="请选择分类"
              v-model="infoForm.specification_id"
            >
              <el-option
          ...
          <el-form-item label="展示缩略图">
            <el-upload
              ...
              :action="api.rootUrl + 'upload/uploadRichText'"
              :show-file-list="false"
              :on-success="handleUploadImageSuccess"
              :headers="uploaderHeader"
            >
              <img
                v-if="infoForm.pic_url"
                :src="infoForm.pic_url"
                class="image-show"
              />
  ...
  methods: {
    ...
    onSubmitInfo() {
      this.$refs["infoForm"].validate(valid => {
        if (valid) {
          this.axios
            .post("specification/goodsSpecAdd", this.infoForm)
            .then(response => {
    ...
    getCatalog() {
      let that = this;
      this.axios
        .post("specification/onlyIndex", { id: this.infoForm.id })
        .then(response => {
