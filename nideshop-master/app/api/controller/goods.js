...
module.exports = class extends Base {
  ...
  listAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      // 搜索到的商品
      const goodsData = yield goodsQuery.where(whereMap).field(['id', 'name', 'list_pic_url', 'retail_price']).order(orderMap).page(page, size).countSelect();
  ...
  countAction() {
    ...
    return _asyncToGenerator(function* () {
      const goodsCount = yield _this10.model('goods').where({ is_delete: 0 }).count('id');
